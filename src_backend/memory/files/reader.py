...
from src_backend.core.errors import MemoryNotFoundError
...
class FileReader:
    ...
    def read_file(self, asset_id: str, selected_file: str) -> File:
        ...
        max_bytes = self.registry.settings.memory_file_max_bytes
        content = path.read_text(encoding="utf-8")
        if path.stat().st_size > max_bytes:
            content = content.encode("utf-8")[:max_bytes].decode("utf-8", errors="ignore")
            content += f"\n...[truncated to {max_bytes} bytes; file is larger]"
        return File(
            asset_id=asset_id,
