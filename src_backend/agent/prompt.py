...
from src_backend.core.text import trim_to_budget
...
def system_prompt(tools: list[ToolSpec], max_iterations: int) -> str:
    ...
    return (
        "You are AssetOS, an agent that solves property related issues by "
        ...
        f"Available tools:\n{tool_catalog}\n\n"
        "Rules:\n"
        "- Unless the task is clearly asset-independent, start by calling list_assets, then read_memory for the "
        "relevant asset before anything else — don't reason about a property from the task text alone.\n"
        ...
        "- Before tackling an unfamiliar task type, call list_skills (and load_skill) to check for a relevant playbook.\n"
        "- Capture durable context that doesn't fit the fact schema (preferences, decisions, open issues) with "
        "save_memory_note or create_asset so future runs have it.\n"
