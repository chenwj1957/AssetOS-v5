...
from src_backend.core.constants import MEMORY_INDEX_DB_FILENAME, MEMORY_SCHEMA_FILENAME
from src_backend.core.errors import LLMProviderError, BaseAppError, RoutingError
from src_backend.core.text import truncate_text
from src_backend.core.types import AgentState, AgentTurn, EventLog
from src_backend.llm.client import LLMClient
from src_backend.memory.assets import AssetRegistry, AssetWriter
...
from src_backend.memory.skills import SkillReader, SkillRegistry, SkillWriter
from src_backend.agent.approval import ApprovalGate
from src_backend.agent.journal import GLOBAL_RUNS_DIR_NAME, write_run_journal
from src_backend.agent.prompt import render_transcript, system_prompt
from src_backend.tools.base import MemoryHub, ToolContext, ToolSpec
from src_backend.tools.registry import list_tools
...
class AgentLoop:
    ...
    def __init__(
        ...
        schema_registry = SchemaRegistry(path=self.settings.dir_memory / MEMORY_SCHEMA_FILENAME)
        fact_reader = FactReader(asset_registry=asset_registry, schema_registry=schema_registry)
        memory_index = MemoryIndex(
            db_path=self.settings.dir_memory / MEMORY_INDEX_DB_FILENAME,
            asset_registry=asset_registry,
            global_runs_dir=self.settings.dir_memory / GLOBAL_RUNS_DIR_NAME,
        )
    ...
    def run(
        ...
        if not user_task.strip():
            raise BaseAppError("User task cannot be empty.")
