...
from src_backend.core.errors import LLMProviderError
from src_backend.tools.base import ToolContext, ToolResult, ToolSpec, require_str
...
def _codex_agent(ctx: ToolContext, args: dict[str, Any]) -> ToolResult:
    ...
    if working_dir:
        framed += (
            "\n\nByproduct: if completing this task involves deeply reading a file under "
            "Files/ that has no matching 'Files/<name>.md' extract yet, also write "
            "'Files/<name>.md' (a Markdown extract of its key content) and "
            "'Files/<name>.md.meta.json' (JSON with a 1-2 sentence \"summary\" field) so future "
            "searches can find it. Skip this if such an extract already exists or the file isn't relevant."
        )
    return _delegate_to_codex(
        ctx,
        framed,
